"""Small synchronous client for cc-jukebox's HTTP MCP server."""
...
MCP_URL_ENV = "CC_JUKEBOX_MCP_URL"
...
def configured_url() -> str:
    env_url = os.environ.get(MCP_URL_ENV, "").strip()
    if env_url:
        return env_url
    try:
    ...
    *,
    ...
    try:
        ...
        raise MCPClientError(
            f"cc-jukebox MCP server is not reachable at {target}: {e}\n"
            "Start it with: cc-jukebox server"
        ) from e
