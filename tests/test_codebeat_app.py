...
def test_codebeat_app_keeps_dock_icon_visible():
    with (
        ...
        load_settings.return_value.petdex_slug = "boba"
        assert app.run() == 0

    run_pet_app.assert_called_once_with(petdex_slug="boba", hide_dock=False, show_control=False)


def test_codebeat_app_defaults_to_boba_when_saved_pet_is_empty():
    with (
        mock.patch("coding_with_beat.app.load_settings") as load_settings,
        mock.patch("coding_with_beat.app.run_pet_app", return_value=0) as run_pet_app,
    ):
