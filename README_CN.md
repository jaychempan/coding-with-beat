...
- **桌面宠物**（开发预览）— `cwb pet` 会在 macOS 桌面打开一个可拖动、置顶的像素小人，内置 5 套皮肤，支持心情输入、歌曲推荐和 coding 状态反应。
...
>
> **桌面宠物：** 在 macOS 上运行 `cwb pet` 打开可拖动像素小人。双击会打开可滚动的 CodeBeat DJ Profile 面板，右键可以打开面板、控制播放、切换宠物和退出。
>
> **Apple Music：** 首次播放目录曲目会弹窗，点击**加入资料库**后再重复播放指令即可。

### 可选桌面宠物依赖

桌面宠物使用 PySide6。安装 GUI 可选依赖后即可启动：
...
面板打开时也会作为 live cockpit 使用：背景和信号有轻微动态，显示当前歌曲、进度、当前歌词行。
...
/cwb pet                  # 桌面像素宠物（macOS，需要 PySide6）
